...
def convert_to_text(month):
    lista1 = [
        'Ano Todo', 'Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro']
    return lista1[month]
...
app.layout = dbc.Container(children=[
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    dbc.Row([
                        dbc.Col([
                            html.Legend("Relatório de Vendas", style={'font-size': '120%'})
                        ], sm=8),
                        dbc.Col([
                            html.I(className='fa fa-balance-scale', style={'font-size': '160%'})
                        ], sm=4, align='center')
                    ]),
                    dbc.Row([
                        dbc.Col([
                    ...
                    dbc.Row([
                        dbc.Button("Visite meu perfil:", href="https://www.linkedin.com/in/eduardo-alberto-somacal/", target="_bkank")
                    ], style={'margin-top': '10px'})
                ])
            ], style=tab_card)
        ], sm=4, lg=2),
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    dbc.Row([
                        dbc.Col([
                            html.Legend('Top Representantes por Estado')
                        ])
                    ]),
                    dbc.Row([
                        dbc.Col([
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H4("Valores de propaganda convertidos no mês"),
...
def graph3(team, toggle):
    ...
    fig3 = go.Figure(go.Scatter(x=df_3["Dia"], y=df_3["Chamadas Realizadas"], mode="lines", fill="tonexty"))
...
def graph4(team, toggle):
    ...
    fig4 = go.Figure(go.Scatter(
        x=df_4['Mês'], y=df_4['Chamadas Realizadas'], mode="lines", fill="tonexty"))
...
def graph5(month, toggle):
    ...
    fig5.add_trace(go.Indicator(mode='number+delta',
        title = {"text": f"<span style='font-size:110%'>{df_5['Consultor'].iloc[0]}<br>Top Consultant</span><br><span style='font-size:70%'>Em vendas - em relação a média</span><br>"},
        value = df_5['Valor Pago'].iloc[0],
        number = {'prefix': "R$"},
        delta = {'relative': True, 'valueformat': '.1%', 'reference': df_5['Valor Pago'].mean()}
    ))

    df_6 = df.groupby("Equipe")["Valor Pago"].sum()
    df_6.sort_values(ascending=False, inplace=True)
    df_6 = df_6.reset_index()
    fig6 = go.Figure()
    fig6.add_trace(go.Indicator(mode='number+delta',
            title = {"text": f"<span style='font-size:110%'>{df_6['Equipe'].iloc[0]}<br>Top Team</span><br><span style='font-size:70%'>Em vendas - em relação a média</span><br>"},
    ...
    fig5.update_layout({"margin": {"l":0, "r":0, "t":55, "b":0}})
    fig6.update_layout({"margin": {"l":0, "r":0, "t":55, "b":0}})
    return fig5, fig6
...
def graph7(toggle):
    ...
    fig7.add_trace(go.Scatter(y=df_7_group["Valor Pago"], x=df_7_group["Mês"], mode='lines+markers', fill='tonexty', fillcolor='rgba(255,0,0,0.2)', name='Total de Vendas'))
...
def graph8(month, toggle):
    ...
    fig8.update_layout(main_config, height=410, template=template)
    return fig8
...
def graph9(month, team, toggle):
    ...
    df_9 = df.groupby("Meio de Propaganda")["Valor Pago"].sum().reset_index()

    fig9 = go.Figure()
    fig9.add_trace(go.Pie(labels=df_9["Meio de Propaganda"], values=df_9["Valor Pago"], hole=.6))
